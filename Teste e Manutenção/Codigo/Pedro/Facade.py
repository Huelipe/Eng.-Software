...
class ContaService:
            ...
            with open(self._FILE_NAME, 'r') as f:
                ...
                for conta_id, dados_conta in dados_salvos.items():
                    ...
                    self._contas_db[conta_id] = Conta(
                        ...
                        saldo_atual=Decimal(dados_conta['saldo_atual']),
                        password_hash=dados_conta.get('password_hash', None),
                        # Lendo novos campos (se não tiver, fica vazio)
                        pergunta_seguranca=dados_conta.get('pergunta_seguranca', ""),
                        resposta_seguranca_hash=dados_conta.get('resposta_seguranca_hash', "")
                    )
    ...
    def _save_data(self):
        ...
        for conta_id, conta_obj in self._contas_db.items():
            dados_para_salvar[conta_id] = {
                ...
                'saldo_atual': str(conta_obj.saldo_atual),
                'password_hash': conta_obj.password_hash,
                # Salvando os novos campos
                'pergunta_seguranca': conta_obj.pergunta_seguranca,
                'resposta_seguranca_hash': conta_obj.resposta_seguranca_hash
            }
    ...
    # Mude a definição da função para receber os novos parâmetros
    def criar_conta(self, id_conta, username, nome_completo, saldo_inicial, senha_plana, pergunta, resposta_plana):
        ...
        # Hash da resposta para segurança (igual senha)
        hash_da_resposta = self._hash_password(resposta_plana.lower().strip()) 
        ...
        nova_conta = Conta(
            ...
            resposta_seguranca_hash=hash_da_resposta
        )
    ...
    def validar_resposta_e_atualizar_senha(self, username, resposta_plana, nova_senha_plana):
        conta = self._get_conta_por_username(username)
        if not conta:
            return False
            
        # Verifica se a resposta bate
        hash_tentativa = self._hash_password(resposta_plana.lower().strip())
        if hash_tentativa == conta.resposta_seguranca_hash:
            # Se bateu, troca a senha
            conta.password_hash = self._hash_password(nova_senha_plana)
            self._save_data()
            return True
        return False

    def get_pergunta_seguranca(self, username):
        conta = self._get_conta_por_username(username)
        if conta:
            return conta.pergunta_seguranca
        return None
...
class FinanceFacade:
    ...
    # Mude o criar_nova_conta para aceitar os parametros novos
    def criar_nova_conta(self, username, nome_completo, senha, pergunta, resposta):
        novo_id = f"conta-{uuid.uuid4().hex[:6]}"
        try:
            return self._conta_service.criar_conta(
                id_conta=novo_id,
                username=username,
                nome_completo=nome_completo,
                saldo_inicial=0,
                senha_plana=senha,
                pergunta=pergunta,      # Enviando
                resposta_plana=resposta # Enviando
            )
        except ValueError as e:
            return None

    # Novos métodos para a recuperação de senha
    def buscar_pergunta_seguranca(self, username):
        conta = self._conta_service._get_conta_por_username(username)
        if conta:
            return conta.pergunta_seguranca
        return None

    def resetar_senha(self, username, resposta_plana, nova_senha):
        conta = self._conta_service._get_conta_por_username(username)
        if not conta: return False
        
        # Verifica hash da resposta
        hash_resp = self._conta_service._hash_password(resposta_plana.lower().strip())
        
        if hash_resp == conta.resposta_seguranca_hash:
            # Troca a senha
            conta.password_hash = self._conta_service._hash_password(nova_senha)
            self._conta_service._save_data()
            return True
        return False
